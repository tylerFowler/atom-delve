  ...
  **/
  ...
  render() {
    ...
    $panelContainer.setAttribute(
      'style',
      `font-family: ${atom.workspace.config.settings.editor.fontFamily}`
    );
  ...
  buildTitleBar() {
    ...
    // TOOD: create an icon for this & insert that
  ...
  toggleMinimize() {
    const minimizedSize = '1.75em'; // pulled from `styles/io-panel.less`
  ...
  **/
  ...
  handleVertResize(srcEvt) {
    ...
    const stopDrag = () => {
      $root.removeEventListener('mousemove', doDrag);
      $root.removeEventListener('mouseup', stopDrag);
      this._$panelContainer.classList.remove('dragged');
    };

    this._$panelContainer.classList.add('dragged');
    $root.addEventListener('mousemove', doDrag);
    $root.addEventListener('mouseup', stopDrag);
