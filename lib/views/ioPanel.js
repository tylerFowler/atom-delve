...
export default class DelveIOPanel {
  ...
  **/
  ...
  buildTitleBar() {
    ...
    $titleBarMinimize.classList.add('delve-io-action-minimize');
    $titleBarMinimize.addEventListener('click', this.onMin.bind(this), false);
    ...
    $titleBarMaximize.classList.add('delve-io-action-maximize');
    $titleBarMaximize.addEventListener('click', this.onMax.bind(this), false);
  ...
  onMax() {
    this._$leftPanel.style.setProperty('display', 'initial');
    this._$rightPanel.style.setProperty('display', 'initial');
    this._$resizeHandle.style.setProperty('display', 'initial');
  }
  ...
  handleHorizResize(srcEvt) {
    ...
    let doDrag = (evt) => {
      ...
      if (isLeftDrag) {
        ...
        if (newLeftW <= 250) return;

        this._$leftPanel.style.width = `${startLeftW + evt.clientX - startX}px`;
        this._$rightPanel.style.width = `${startRightW - (evt.clientX - startX)}px`;
      } else {
        let newRightW = startRightW + startX - evt.clientX;
        if (newRightW <= 250) return;

        this._$leftPanel.style.width = `${startLeftW - (startX - evt.clientX)}px`;
        this._$rightPanel.style.width = `${startRightW + startX - evt.clientX}px`;
      }
    };
