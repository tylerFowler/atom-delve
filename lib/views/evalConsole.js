...
/**
  ...
  TODO: implement a recursive fn for pretty printing Go values that are returned
    by Delve, should probably put this in Delvejs
**/
...
const DelveExprVal = createJSX({
  renderView(_val) {
    ...
    return <pre className="delve-go-value">{val.toString()}</pre>;
  }
});
...
export default class EvalConsole {
  constructor() {
    ...
    this.evalConsole.addEventListener('click', () => this.$curInput.focus());
  }
  ...
  sendEvalExpr(evt) {
    ...
    const provideVal = (val, isError) => {
      ...
      if (val.value !== void 0) text = val.value;
      else text = val;
