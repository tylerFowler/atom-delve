...
function addBreakpoints(err, bp) {
  ...
  let idx = _breakpoints.findIndex(b => b.id < 0
    && b.location.file == bp.location.file
    && b.location.line == bp.location.line);
...
class BreakpointStore extends Store {
  ...
  __onDispatch(payload) {
    ...
    case Constants.BREAKPOINT_CREATE_FAILURE:
      ...
      else failedBpData = { name: payload.name, location: payload.location };
    ...
    default: didChange = false;
    }

    if (didChange) this.__emitChange();
  }
}
