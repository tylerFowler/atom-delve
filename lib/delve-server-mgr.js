...
export default class DelveServerMgr extends EventEmitter {
  ...
  **/
  spawnProcess(command, cwd, buildFlags) {
    ...
    let args = [
      command, '--headless', `--listen=${serverHost}:${serverPort}`,
      '--log=true'
    ];
    ...
    let detectMsgTypeAndRedirect = d => {
      ...
      if (msg.trim().search(/^[0-9]{4}.*$/) < 0) {
        if (!this.hasSessionStarted) this.emit('error', new Error(msg));
    ...
    this.delveProc.stdout.on('data', d => {
      if (!this.hasSessionStarted) this.emit('error', new Error(d.toString()));
      else this.emit('runtimeStdout', d);
    });
