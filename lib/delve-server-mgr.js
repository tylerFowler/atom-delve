...
export default class DelveServerMgr extends EventEmitter {
  ...
  endSession() {
    ...
    // TODO: remove the 'debug' file at the proj root just like delve does
  }
  ...
  **/
  spawnProcess(command, cwd, buildFlags) {
    ...
    let detectMsgTypeAndRedirect = d => {
      let msg = d.toString();
      // TODO: std out from the program being run will look like this as well,
      // can we figure out an error without using the timestamp then send
      // non Delve-log output to another pane for stdout?
      // TODO 2: perhaps just create a pane for *all* Delve output instead
      // of trying to decide if it's an error
