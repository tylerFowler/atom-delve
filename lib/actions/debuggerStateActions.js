...
export default {
  ...
  **/
  ...
  /**
    @private
    @name State#_getGoroutines
    @desc gets a list of the current active goroutines
    @returns Promise<goroutine[]>
  **/
  _getGoroutines() {
    return Delve.getGoroutines()
    .then(goroutines => { return !!goroutines ? goroutines : []; });
  },
  ...
  **/
  continueToBreakpoint() {
    Promise.all([ Delve.continueToNextBreakpoint(), this._getGoroutines() ])
    .spread((debuggerState, goroutines) =>
      this._getStacktrace(debuggerState.currentGoroutine)
      .then(stacktrace =>
        Dispatcher.dispatch({
          type: Constants.DEBUGGER_CONTINUE,
          debuggerState, stacktrace, goroutines
        })
      )
  ...
  **/
  nextInScope() {
    Promise.all([ Delve.next(), this._getGoroutines() ])
    .spread((debuggerState, goroutines) =>
      this._getStacktrace(debuggerState.currentGoroutine)
      .then(stacktrace =>
        Dispatcher.dispatch({
  ...
  **/
  step() {
    Promise.all([ Delve.step(), this._getGoroutines() ])
    .spread((debuggerState, goroutines) =>
      this._getStacktrace(debuggerState.currentGoroutine)
      .then(stacktrace =>
        Dispatcher.dispatch({
  ...
  **/
  stepInstruction() {
    Promise.all([ Delve.stepInstruction(), this._getGoroutines() ])
    .spread((debuggerState, goroutines) =>
      this._getStacktrace(debuggerState.currentGoroutine)
      .then(stacktrace =>
        Dispatcher.dispatch({
